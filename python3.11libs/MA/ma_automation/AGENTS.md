...
- **数据持久化**：所有槽状态序列化为 `list[dict]`，**仅在点击 执行 时落盘**（`_start_execution` → `_save_data`），关窗不保存。语义：JSON = 用户决定执行的任务，不是当前 UI 状态；编辑后未点 执行 直接关窗 = 丢弃未执行编辑（有意为之）。加载在 `__init__._load_data()`
  - **副作用契约**：`DataManager` 三方法严格分离副作用 —— `get_data_path()` 纯计算不创建目录、`load()` 纯只读（文件/目录不存在时返回 `[]`，不创建任何东西）、`save()` 是**唯一**允许创建配置目录的入口（`os.makedirs(exist_ok=True)` 在写入前）。这保证"打开面板 + 编辑 + 关闭 = 0 文件副作用"，配置目录和 JSON 文件只在点 执行 时才出现
  - **多文件支持**:`get_data_path(filename)` / `load(filename)` / `save(data, filename)` 三方法均接受可选文件名(无 `.json` 后缀),`filename=None` 走默认 `MA_Automation.json`(完全向后兼容)。配合 `list_configs()` 列出配置目录下所有 `.json` 文件 basename(无后缀,sorted,纯只读不创建目录),让 UI 提供"可编辑配置下拉"——用户可选已有配置 / 键入新名 + 执行 创建新文件
...
- **可取消**：`cancel()` 置 `_cancelled` 标志，`run()` 在任务间隙（`msleep(100)`）检查；执行按钮在 执行 / 取消 文案间切换
...
- **Flipbook 用户参数**：`FlipbookParams` 包含用户可配置字段（`start_frame` / `end_frame` / `output_path` / `save_to_disk`），默认值为 `$RFSTART` / `$RFEND` / `$HIP/FlipBook/$HIPNAME/$HIPNAME.$F4.jpg` / `True`。`_execute_flipbook` 中帧范围通过 `hou.text.expandString()` 展开后写入 `settings.frameRange()`；输出路径**不展开**（保留 `$F4` 表达式），直接传给 `settings.output()`，由 Houdini 逐帧展开
- **Flipbook 独立 widget**：Flipbook 的参数区（帧范围 + 输出路径）独立于 `QStackedWidget`，通过 `show/hide` 控制显隐。原因：`QStackedWidget` 所有页面强制共享最大页面宽度，Flipbook 参数较多会导致按钮点击和 Webhook 页面被拉宽。stacked widget 仅包含 BUTTON_CLICK 和 HOME_ASSISTANT（宽度相近）
- **Flipbook 打开文件夹按钮**：输出路径旁有 `folder.svg` 图标按钮，点击后用 `hou.text.expandString()` 展开路径，取父目录，`subprocess.Popen(["explorer", ...])` 打开。路径不存在时弹 `QMessageBox.warning`
- **`dl_Submit` 特殊处理**：`parm_name == "dl_Submit"` 时按钮按下后自动 `hou.hipFile.save()`（`ExecutionEngine._execute_button_click`）
- **工具栏中文按钮**：执行（`startBtn`）、自动填充（`autoFillBtn`）、清空（`clearBtn`）、设置（`settingsBtn`）、取消（执行中切换为取消文案）
- **样式**：`STYLE_SHEET` 字符串集中定义在 `styles.py`，复用项目暗色主题 `#18181b` / `#0d6399`；任务槽卡片化用 `QWidget#taskSlot { background-color: #252528; }`（**直角无圆角**）+ `setAutoFillBackground(True)`；关键 objectName：`startBtn`（蓝色执行按钮）、`addBtn` / `removeBtn`（± 任务槽增减，padding 8px、字号 16px）、`taskSlot`（任务槽卡片容器）、`taskSlotHandle`（序号手柄，单击选中 / 按住拖动重排）
...
- **`startBtn` 必须 `setAutoDefault(False)` + `setDefault(False)`**:`QDialog` 默认 Enter 触发 default 按钮,而 `QPushButton` 默认 `autoDefault=True`,用户在可编辑 `_config_combo` 键入新名按 Enter 提交文字时,会被错误地转成 执行 触发。**要求 执行 只能手动鼠标点击**(键盘 Enter 不应触发任何执行动作,只是 commit combo 文本)。`autoFill` / `clear` / `+` / `-` 同样建议关闭 autoDefault,保持一致性
...
- **可编辑配置下拉 `_config_combo`**:工具栏在 `startBtn` **前**方放一个 `QLabel("配置:")` + `QComboBox`(`setEditable(True)` + `setInsertPolicy(NoInsert)` + `setPlaceholderText("选择 / 键入配置名")`),让用户选择/键入配置文件名。
  ...
  - **选 vs 键入**:**选择**已有项(`currentIndexChanged`)→ 立即重新加载该文件覆盖面板;**键入**新名(无匹配项)→ **不立即加载**,`执行` 时 `_save_data` 把当前面板状态写到 `{name}.json`(不存在则创建;键入空 / 全空白 / 含路径分隔符 → fall back 到默认 `MA_Automation.json`)
