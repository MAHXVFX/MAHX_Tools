...
- **可编辑配置下拉 `_config_combo`**:工具栏在 `startBtn` **前**方放一个 `QLabel("配置:")` + `QComboBox`(`setEditable(True)` + `setInsertPolicy(NoInsert)` + `setPlaceholderText("选择 / 键入配置名")`),让用户选择/键入配置文件名。
  ...
  - **回归测试**:`TestConfigComboSaveTarget` 15 case(sanitize 6 + 边界 3 REJECT + 3 KEEP)+ `TestConfigComboRefresh` 5 case + `TestConfigComboSelectionChange` 3 case + `TestSaveUsesConfigName` 5 case(成功 4 + 失败 1)+ `TestConfigComboEndToEnd` 2 case(save 成功/失败 端到端)+ `TestConfigComboSourceContract` 6 case(源码契约:combo 存在 / 3 helper 含 `quote(safe='/:')` / label 存在且在 combo 前 / styles.py 专属样式 / SVG 文件存在 + Path(__file__) 注入 / startBtn setAutoDefault+setDefault False 锁死)+ `TestConfigFileSelection` 17 case(DataManager 层),共 **53 个 case** 锁死
  - **SVG URL 必须 `quote(safe='/:')` 编码**:`_CONFIG_COMBO_ICON_STYLE` 用 `urllib.parse.quote(_ICON_DROP_DOWN.as_posix(), safe='/:')` 编码 SVG 绝对路径再嵌入 `url(...)`。**不可裸路径** —— Win32 用户目录常含空格(`C:\Users\mahx_\Documents\...`),裸 `url(C:\Users\...)` 让 QSS 解析器在空格处截断,图标不显示。`safe='/:'` 保留盘符冒号 + 路径分隔符,只编码空格/中文等不安全字符。回归测试 `TestConfigComboSourceContract.test_helper_methods_in_source` 锁死源码含 `quote(... safe='/:')`
