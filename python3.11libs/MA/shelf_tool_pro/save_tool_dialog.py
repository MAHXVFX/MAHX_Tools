...
class ToolSettingsDialog(QtWidgets.QDialog):
    ...
    def _build_ui(self, tool_name: str, label: str) -> None:
        ...
        self._tab_widget.setStyleSheet(
            f"QTabWidget::pane {{ border: 1px solid {BORDER_COLOR}; border-radius: 0 0 4px 4px; background-color: {BG_SECONDARY}; }}"
            f"QTabBar {{ left: 8px; }}"
            f"QTabBar::tab {{ background-color: {BG_PRIMARY}; color: {TEXT_SECONDARY}; "
            f"padding: 8px 20px; border: 1px solid {BORDER_COLOR}; "
            f"border-top-left-radius: 4px; border-top-right-radius: 4px; margin-right: -1px; margin-bottom: -1px; }}"
            f"QTabBar::tab:last {{ margin-right: 0; }}"
