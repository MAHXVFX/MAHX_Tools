...
显示插件信息的关于面板，使用 QTextBrowser 渲染 HTML 内容。

## Where to Look

| File | Purpose |
|------|---------|
| `about_panel.py` | 关于面板 UI：单例对话框、QTextBrowser 渲染、Win32 窗口样式 |

## Conventions

- **QTextBrowser 渲染**：使用 `QTextBrowser` 渲染 HTML 内容，轻量级原生渲染，无延迟显示
...
- **HTML 内容**：`ABOUT_HTML` 常量定义完整的关于信息（版本、功能、技术栈）
- **窗口属性**：`Qt.WA_DeleteOnClose` 属性，关闭时自动清理 `_panel_window`
- **外部链接**：`QTextBrowser.setOpenExternalLinks(True)` 支持点击打开外部链接
